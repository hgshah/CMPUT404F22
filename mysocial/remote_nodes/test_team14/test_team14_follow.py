...
class Team14Session:
    ...
    def __init__(self, auth: (str, str), base='http://127.0.0.1:8014'):
        self.auth = auth
        self.base = base
        self.session = requests.Session()
        # self.session.auth = self.auth
        response = self.session.post(f'{self.base}/login/', {"username": auth[0], "password": auth[1]})
        print("ok")
        # self.content = json.loads(response.content.decode('utf-8'))
        # self.author_id = self.content['id']
...
class TestTeam14Follow(TestCase):
    def setUp(self):
        ...
        self.remote = Team14Session(('super', 'super'), 'http://127.0.0.1:8014')
